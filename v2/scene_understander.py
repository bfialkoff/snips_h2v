...
class SceneUnderstander:
    ...
    def get_focus_points_single_human(self):
        h, w = self.scene[0][1].shape[:2]
        focus_points = []
        last_focus = None
    ...
    def get_focus_points(self, target_ratio=9/16):
        ...
        assert len(focus_points) == len(self.scene)
        scene_duration = self.scene[-1][0] - self.scene[0][0]
        smooth_focus_points = self.smooth_focus_points(focus_points, scene_duration, )
        assert len(smooth_focus_points)
        return smooth_focus_points, scene_type